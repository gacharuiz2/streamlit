...
st.sidebar.title('¿Tus datos personales?')
edad= st.sidebar.slider(label="¿Cuál es tu edad?",min_value=1,max_value=100,value=18,step=1)
genero= st.sidebar.selectbox(label="¿Cuál es tu genero?",options=['Masculino','Femenino'],index=0,)
region= st.sidebar.selectbox(label="¿En qué region vives?",options=['Lima','Lambayeque','Callao'],index=0,)
distrito= st.sidebar.selectbox(label="¿En qué distrito vives?",
	options=['seleccionar','CHICLAYO','SJL','CALLAO','SMP','CERCADO','VILLA EL SALVADOR','ATE','BREÑA',
	         ...
	         'SAN BORJA','SURQUILLO','SAN MIGUEL','INDEPENDENCIA','SAN ISIDRO','PUENTE PEIDRA'],index=0,)


st.title("Test de riesgo Covid-19")
...
<h2 style="color:white;text-align:center;">Nivel de exposición al Covid19: </h2>
</div>
"""
st.markdown(html_temp,unsafe_allow_html=True)

st.write('**[1] ¿Sales de casa?**')
sales=st.selectbox(label="Responde1",options=['Nunca','Algunas veces','Frecuentemente'],index=0,)

st.write('**[2] ¿Has estado últimamente en una zona de riesgo como: trasporte publico,supermercados,tiendas u bancos?**')
publico=st.selectbox(label="Responde2",options=['Si','No'],index=0,)

st.write('**[3]¿Cual de estas medidas tomas al salir?**')
medidas=st.multiselect(label="Responde3",options=['Nunca salgo','Mascarilla','Guantes','Lentes','Mameluco','Ducha al regresar'])

st.write('**[4]¿Has estado o crees haber estado en contacto directo con algún contagiado de COVID-19?**')
contacto=st.selectbox(label="Responde4",options=['Si','No'],index=0,)

st.write('**[5]¿Tienes algún familiar policia, militar, medico, enfermero?**')
familiar=st.selectbox(label="Responde5",options=['seleccionar','Si','No'],index=0,)
...
html_temp = """
<div style="background-color:tomato;padding:10px">
<h2 style="color:white;text-align:left;">Describe tus síntomas </h2>
</div>
"""
st.markdown(html_temp,unsafe_allow_html=True)

st.write('**[6]¿Presentas algunos de estos sintomas?**')
sintomas=st.radio(label="Responde6",options=['Ninguno','Tos seca/dolor garganta/dolor muscular','Tos con flema/dolor garganta/dolor muscular','Estornudo/mocosidad nasal/malestar corporal'],index=0,)

st.write('**[7]¿Sensación de falta de aire o dificultad para respirar aún estando en reposo?**')
aire=st.selectbox(label="Responde7",options=['Si','No'],index=0,)

st.write('**[8]¿Coloración azul en los labios(cianosis)?**')
labio=st.selectbox(label="Responde8",options=['Si','No'],index=0,)

st.write('**[9]¿Fiebre mayor a 38° persistente por más de dos días?**')
fiebre=st.selectbox(label="Responde9",options=['Si','No'],index=0,)

st.write('**[10]¿Desorientación y confusión?**')
confusion=st.selectbox(label="Responde10",options=['Si','No'],index=0,)
...
"""
def main():
	if st.button("nivel de contagio"):
		result1 = contagio()
		result2 = riesgo()
		result1
		result2
